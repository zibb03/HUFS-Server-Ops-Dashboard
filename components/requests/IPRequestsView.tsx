...
interface Props { initialRows: IPRequestRow[] }

export default function IPRequestsView({ initialRows }: Props) {
  const isAdmin = useAdmin()
  const [tab, setTab]   = useState<'list' | 'manage'>('list')
  const [open, setOpen] = useState(false)
  const [rows, setRows] = useState<IPRequestRow[]>(initialRows)
  ...
  // 서버에서 받은 initialRows는 SSR로 첫 화면에 즉시 표시.
  // 변경(POST/PATCH/DELETE) 후엔 client fetch로 갱신.
  const load = () =>
