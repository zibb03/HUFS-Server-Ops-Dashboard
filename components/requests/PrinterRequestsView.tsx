...
import type { PrinterRequestRow, RequestStatus } from '@/lib/types'

const PAGE_SIZE = 20

interface Props { initialRows: PrinterRequestRow[] }

export default function PrinterRequestsView({ initialRows }: Props) {
  const isAdmin = useAdmin()
  const [tab, setTab]   = useState<'list' | 'manage'>('list')
  const [open, setOpen] = useState(false)
  const [rows, setRows] = useState<PrinterRequestRow[]>(initialRows)
  ...
  const handleCsv = () => {
    downloadCsv(`printer_requests_${todayLocal()}.csv`, filtered, [
      { key: 'id', label: '번호' },
      { key: 'applicant_name', label: '신청자명' },
      { key: 'printer_id', label: '프린터' },
      { key: 'copies', label: '출력 매수' },
      { key: 'status', label: '상태' },
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-surface-low">
                ...
                {['#', '신청자명', '프린터', '출력 매수', '신청일', ...(tab === 'manage' ? ['상태', '처리'] : [])].map(h => (
            ...
            <tbody>
              ...
              {paged.length === 0 && (
                <tr><td colSpan={tab === 'manage' && isAdmin ? 8 : 5} className="px-4 py-8 text-center text-secondary text-sm">신청 내역이 없습니다.</td></tr>
              )}
            </tbody>
          </table>
        </div>
